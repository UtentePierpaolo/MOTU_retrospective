...
# # only falls with prosthesis-----------------
# 
# table(Fall$WearingProsthesis, useNA="a")
# 
# 
# # logistic model, so far without adjusting for length of stay
# cformula <- "dfallwp ~  KneeCategory + (1 | PatientID)"
# m1 <- glmer(cformula, data=dfm, family = binomial(link = "logit"), nAGQ = 20)
# summary(m1)
# 
# # conditioning on FirstdeliveryRenewal
# cformula <- "dfallwp ~  KneeCategory + FirstdeliveryRenewal + (1 | PatientID)"
# m1 <- glmer(cformula, data=dfm, family = binomial(link = "logit"), nAGQ = 20)
# summary(m1)
...
# nfall vs time since amputation
nbreak=5
Days2Years <- 356.2
dfm$YearsFromAmputation <- dfm$TimeFromAmputation/Days2Years
# breaks <- quantile(x=dfm$YearsFromAmputation, seq(from=0,to=1,length.out=nbreak+1), na.rm=T)
breaks= c(0,1,3,10,30,80)

dfm$TimeFA <- cut(x=dfm$YearsFromAmputation, include.lowest=TRUE, breaks=breaks)
# dfm$TimeFA <- ordered(dfm$TimeFA, levels=c("[28,371]","(371,1.88e+03]","(1.88e+03,6.66e+03]","(6.66e+03,1.29e+04]","(1.29e+04,2.74e+04]"))

# mean time from amputation according to group
# meangTimeFA <- aggregate(YearsFromAmputation ~ TimeFA, data=dfm, FUN=mean)

cformula <- "nfall  ~ -1 + offset(log(LengthOfStay)) + TimeFA + (1|PatientID)"
cdfm <- na.omit(dfm[,c("nfall","LengthOfStay","TimeFA","PatientID")])
m1 <- glmer(cformula, data=cdfm, family = poisson(link = "log"), nAGQ = 20)
...
vsted <- a$coefficients[,"Std. Error"]
ndays <- 1000
irr <- ndays*exp(vc)
irr_u <- ndays*exp(vc+ vsted) 
irr_l <- ndays*exp(vc - vsted)
nvar <- levels(cdfm$TimeFA)
data.estimates <- data.frame(irr=irr, irr_u=irr_u, irr_l=irr_l, var=nvar)

png("C:/Users/Pierpaolo/OneDrive - Alma Mater Studiorum Università di Bologna/MOTU/retrospective study/Figures/FallsTimeFromAmput.png", 
    width =6,height =5, units = 'in', res = 300)
x <- (0:(length(breaks)-2))/2
plot(x, irr, ylim=c(0,5.5), xaxt='n', 
     xlab="Time from amputation (years)", ylab="Number of falls per 1000 days",
     cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,)
axis(side = 1, at = x, labels=nvar, cex.lab=1.5, cex.axis=1.5)
#points(expect/nsub, obs/nsub, col="red")
errbar(x,irr,irr_u,irr_l,add=T,lwd=2, errbar.col="blue")
lines(x, irr, col="blue")
dev.off()



# nfall vs number of comorbidities
nbreak <- 5
breaks <- quantile(x=dfm$nComorbidities, seq(from=0,to=1,length.out=nbreak+1), na.rm=T)
dfm$nCO <- cut(x=dfm$nComorbidities, include.lowest=TRUE, breaks=breaks)
# dfm$TimeFA <- ordered(dfm$TimeFA, levels=c("[28,371]","(371,1.88e+03]","(1.88e+03,6.66e+03]","(6.66e+03,1.29e+04]","(1.29e+04,2.74e+04]"))

# mean time from amputation according to group
# meangTimeFA <- aggregate(YearsFromAmputation ~ TimeFA, data=dfm, FUN=mean)

cformula <- "nfall  ~ -1 + offset(log(LengthOfStay)) + nCO + (1|PatientID)"
cdfm <- na.omit(dfm[,c("nfall","LengthOfStay","nCO","PatientID")])
m1 <- glmer(cformula, data=cdfm, family = poisson(link = "log"), nAGQ = 20)
...
irr_u <- ndays*exp(vc+ vsted) 
irr_l <- ndays*exp(vc - vsted)
nvar <- levels(cdfm$nCO)
data.estimates <- data.frame(irr=irr, irr_u=irr_u, irr_l=irr_l, var=nvar)

png("C:/Users/Pierpaolo/OneDrive - Alma Mater Studiorum Università di Bologna/MOTU/retrospective study/Figures/FallsNComorb.png", 
    width =6,height =5, units = 'in', res = 300)
x <- (0:(length(breaks)-2))/2
plot(x, irr, ylim=c(0,5.5), xaxt='n', 
     xlab="Number of comorbidities", ylab="Number of falls per 1000 days",
     cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,)
axis(side = 1, at = x, labels=nvar, cex.lab=1.5, cex.axis=1.5)
#points(expect/nsub, obs/nsub, col="red")
errbar(x,irr,irr_u,irr_l,add=T,lwd=2, errbar.col="blue")
...
# nfall vs number of drugs
nbreak <- 4
# breaks <- quantile(x=dfm$nDrugs, seq(from=0,to=1,length.out=nbreak+1), na.rm=T)
breaks <- c(0,1,3,7,30)
dfm$nDr <- cut(x=dfm$nDrugs, include.lowest=TRUE, breaks=breaks)

cformula <- "nfall  ~ -1 + offset(log(LengthOfStay)) + nDr + (1|PatientID)"
cdfm <- na.omit(dfm[,c("nfall","LengthOfStay","nDr","PatientID")])
m1 <- glmer(cformula, data=cdfm, family = poisson(link = "log"), nAGQ = 20)
...
irr_u <- ndays*exp(vc+ vsted) 
irr_l <- ndays*exp(vc - vsted)
nvar <- levels(dfm$nDr) # c("0","1-2","3-7","8-30")
data.estimates <- data.frame(irr=irr, irr_u=irr_u, irr_l=irr_l, var=nvar)

png("C:/Users/Pierpaolo/OneDrive - Alma Mater Studiorum Università di Bologna/MOTU/retrospective study/Figures/FallsNDrugs.png", 
    width =6,height =5, units = 'in', res = 300)
x <- 1:length(nvar)
plot(x, irr, ylim=c(0,5.5), xaxt='n', 
     xlab="Number of drugs", ylab="Number of falls per 1000 days",
     cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,)
axis(side = 1, at = x, labels=nvar, cex.lab=1.5, cex.axis=1.5)
#points(expect/nsub, obs/nsub, col="red")
errbar(x,irr,irr_u,irr_l,add=T,lwd=2, errbar.col="blue")
